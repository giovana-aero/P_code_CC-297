function f_res_log(address_list,casename,colors,linestyles,lgds,ref_val,fontsize,lnwdth,pos,pba,xlims,ylims,save_figs,filename)

% figure(1),clf,grid on,hold on
hold on,grid on
for i = 1:length(address_list)
  % res =readmatrix([address_list{i},casename,'_',xy,'.log'],'filetype','delimitedtext');
  res = dlmread([address_list{i},casename,'.log']);
  disp([address_list{i},casename,'.log'])
