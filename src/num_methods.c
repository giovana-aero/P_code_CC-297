  ...
  for(iter;iter<=config->max_iter;iter++){
    ...
    for(int j=1;j<m-1;j++){
      for(int i=1;i<n-1;i++){
        ...
        vars_old = -L_phi + (phi_old[j][i-1] - 2.*phi_old[j][i])/dx2[i] + (phi_old[j-1][i] - 2.*phi_old[j][i])/dy2[j];
        phi[j][i] = (vars_old*dx2[i]*dy2[j] - dy2[j]*phi[j][i-1] - dx2[i]*phi[j-1][i])/(-2.*(dx2[i] + dy2[j]));

        val = (phi[j][i-1] - 2.*phi[j][i])/dx2[i] + (phi[j-1][i] - 2.*phi[j][i])/dy2[j] - (phi_old[j][i-1] - 2.*phi_old[j][i])/dx2[i] - (phi_old[j-1][i] - 2.*phi_old[j][i])/dy2[j];
        val = fabs(val);
        if(val > res[iter])
          res[iter] = val;
      }
